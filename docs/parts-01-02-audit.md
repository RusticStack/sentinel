...
| High | `hash_files` is not fully bounded against hostile/mutable filesystem input | `hash_files.rs::read_dir` collects an entire directory before applying match limits; recursive `**` traversal has no visited-entry/depth budget; byte accounting trusts initial metadata while the read loop continues to EOF; symlink checks and later opens are separate operations. | **C06 reopened.** Bound traversal, actual streamed bytes and depth; use race-resistant no-follow file access or enforce a genuinely immutable/private input tree; fail on traversal/read failures instead of treating them as absent files. Verify adversarial trees and concurrent replacement before W03/K03 use. This does not block A01. |
...
The [TODO tracker](../TODO.md) carries these gates. Historical completion-log rows remain historical evidence; the reopened C06 checkbox reflects the current audit result.
