...
| Medium | Store resource bounds lag the F04 executors | `Store::read` opened another connection whenever its pool was empty. `Writer::raw` waited indefinitely, `Writer::drop` joined indefinitely, and a panicking closure terminated the writer thread. | **Closed (commit `fix: close the Parts 01–02 execution gates`).** [Storage](storage.md#bounds-and-failure-behavior): at most `READER_LIMIT` readers with a bounded admission wait that sheds `Overloaded`; `WRITE_WAIT` after which a write is reported `WriteAmbiguous` — never as rolled back; a panicking closure is caught, its transaction unwound, the writer keeps serving; `Store::shutdown(timeout)` drains or reports `Stalled` and then keeps ownership. Each case has a wall-clock-asserted test. |
| Medium | Image pinning has no complete durable resolution boundary | `RunSpec::new` accepts tag-only images; `ImageRef::pin` mutated an in-memory value, while stored specs are immutable and there was no durable resolution. | **Closed (same commit).** Migration 12 puts a once-written digest and platform on the job row; `runs::resolve_image` records them (agreeing with a spec pin, refusing malformed values), and `jobs::lease` refuses `Unresolved` work as a predicate on the row it already reads. A rerun keeps the same bytes. Tested for tag-only and digest-pinned specs, API and raw-SQL immutability. |
...
- ~~Before W02, enforce the single-controller ownership contract in process startup~~ — done: `Store::open` takes an OS lock on `<database>.lock` for the life of the store and a second opener gets `AlreadyOwned`; a stalled shutdown keeps the lock until the process exits.
