...
**Reports and specs.** After acknowledging, a worker asks for the run spec (`NeedSpec` → `Spec` chunks of `SPEC_CHUNK_BYTES`, reassembled up to `MAX_SPEC_BYTES`; `NoSpec` when the attempt is not its) and then reports each phase with `Report { attempt, fence, event }`. The controller runs every report through `dispatch::report`: the attempt must be held by that worker under that fence, then the state machine decides; a stale or foreign report changes nothing and is counted in `stale_reports`. A terminal report releases the reservation, decides dependents and wakes the dispatcher. The `Executor` trait the worker implements is in [executor](executor.md). Log frames (`Log`/`LogEnd`, answered with `LogAck`/`LogRefused`) follow the same held-by-this-worker rule and are acknowledged only once durable; see [logs](logs.md).
