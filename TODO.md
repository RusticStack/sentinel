...
- [x] **C06** Add the bounded expression grammar, event/ref filters, concurrency keys, dependency outcome conditions, policy defaults, and `hash_files` evaluation phases/limits. Prevent expressions from becoming arbitrary execution or network calls. See [pipeline schema](docs/pipeline-schema.md#expressions-c06) and completion evidence below.
...
**Next task:** `A01` — tenant namespaces, users/external identities, memberships, repo grants and the super-admin/tenant-admin/operator/reader/service-account authorization model, enforced in repository queries rather than trusting caller-supplied tenant context. This starts the vertical slice (`A01–A03`, then `W01–W09`).
