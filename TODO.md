...
**Next task:** `C05` — compile image/run/steps/env/workdir/resources and DAG dependencies into the immutable per-run spec persisted by the store; reject cycles/missing jobs (done in C04) and specify shell failure semantics and pinned source/image inputs; distinguish rerun from new dispatch.
