...
**Next task:** `C08` — verify transitions and parser limits with malformed-input, duplicate-request, transaction-rollback, conflicting-update and crash-reopen cases; publish the schema/API compatibility policy. Then start the vertical slice at `A01`.
