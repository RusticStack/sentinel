...
Status: F01–F03 complete: Rust workspace, CLI/configuration, Linux process lifecycles, and the contributor workflow. The CI engine and job execution are not implemented yet.
...
**Next task:** `F04` — add structured tracing, correlation IDs, monotonic timing, bounded queues, and explicit blocking/CPU-work boundaries. Continue into the first vertical slice; update this tracker with actual implemented behavior and measurements.
