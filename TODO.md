...
Status: F01–F05 complete: Rust workspace, CLI/configuration, Linux process lifecycles, contributor workflow, tracing/timing/bounded-work foundations, and the benchmark runner with a WSL2 rootless-runtime baseline. The CI engine and job execution are not implemented yet.
...
**Next task:** `F06` — capture representative current CI/Lockwell baseline data and the physical topology behind its runner names; measure isolated versus concurrent commands and separate compile, test, waiting and cgroup throttling. Record unavailable hardware access as a blocker rather than inventing results. Repeat the F05 baseline on a dedicated Linux host when one is available.
