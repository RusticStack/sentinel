...
**Next task:** `C07` — cache/secret/artifact bindings in the schema plus offline `pipeline validate`/`explain` in the CLI with unresolved-input and missing-permission diagnostics; then `C08` malformed-input/transaction/crash-reopen verification and the schema/API compatibility policy. After Part 02, start the vertical slice at `A01`.
