...
Status: Part 01, core/store/protocol/pipeline foundations and A01 durable identity/authorization implemented. The [Parts 01–02 audit](docs/parts-01-02-audit.md) reopened C06's filesystem-resolver boundedness and records store-lifetime/image-resolution gates before execution. WSL2 benchmark records are development baselines. Login/session authentication and job execution are not implemented yet.
...
- [ ] **C06** Add the bounded expression grammar, event/ref filters, concurrency keys, dependency outcome conditions, policy defaults, and `hash_files` evaluation phases/limits. Prevent expressions from becoming arbitrary execution or network calls. Grammar/compiler implemented; **reopened by the 2026-09-14 audit:** finish bounded filesystem traversal/depth/actual read bytes and race-resistant source access before worker use. See [pipeline schema](docs/pipeline-schema.md#expressions-c06), [audit](docs/parts-01-02-audit.md) and historical completion evidence below.
...
**Audit integration gates (2026-09-14):** C02/W02 must bound reader admission/pool retention and define writer panic/drain/ambiguous-wait behavior, and enforce the single-controller startup contract. C05/W03 must durably resolve image digest/platform before executable admission. C06's reopened resolver must be safe and bounded before W03/K03 use. A01 fixes database ownership and future-version rejection. Details and qualification blockers: [Parts 01–02 audit](docs/parts-01-02-audit.md). These gates do not block identity/bootstrap work.
...
**Next task:** `A02` — host-local first-admin bootstrap, maintained password hashing, local login/sessions with expiry/revocation, secure cookies/CSRF and audited recovery; prevent removing the last active super admin. Continue the requested identity slice, and close the recorded audit gates before worker execution.
