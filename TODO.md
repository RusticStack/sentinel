...
Status: F01–F02 complete: Rust workspace, CLI/configuration, and Linux server/worker process lifecycles. The CI engine and job execution are not implemented yet.
...
**Next task:** `F03` — establish developer commands for formatting, lint, tests and release builds, and document Linux runtime prerequisites and the local two-process development setup. Continue into the first vertical slice; update this tracker with actual implemented behavior and measurements.
