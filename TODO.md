...
Status: Part 01, core/store/protocol/pipeline foundations, A01 durable identity/authorization and A02 local authentication implemented. C06's audited resolver gaps are closed with bounded Linux descriptor-rooted hashing. The [Parts 01–02 audit](docs/parts-01-02-audit.md) records remaining store-lifetime/image-resolution gates before execution. WSL2 benchmark records are development baselines. Local login, sessions and host-local bootstrap/recovery exist as library and host-local command surfaces; the HTTP routes that use them (W08) and job execution are not implemented yet.
...
**Next task:** `A03` — host-local provisioned, scoped, expiring development/service credential so the first CLI/API slice is authenticated before browser OAuth exists; hash token secrets and reuse the A01/A02 authorization layer rather than adding a no-auth bypass.
