...
Status: Part 01, core/store/protocol/pipeline foundations, A01 durable identity/authorization, A02 local authentication and A03 scoped API credentials implemented. C06's audited resolver gaps are closed with bounded Linux descriptor-rooted hashing. The [Parts 01–02 audit](docs/parts-01-02-audit.md) records remaining store-lifetime/image-resolution gates before execution. WSL2 benchmark records are development baselines. Local login, sessions, host-local bootstrap/recovery and scoped expiring API credentials exist as library and host-local command surfaces; the HTTP routes that use them (W08) and job execution are not implemented yet.
...
**Next task:** `A04` — GitHub user sign-in and verified identity linking: preserve immutable provider user IDs, validate state and redirects, and keep login tokens separate from installation tokens. Alternatively start `W01` now that an authenticated CLI/API caller exists.
