...
Status: Part 01, core/store/protocol/pipeline foundations, A01 durable identity/authorization, A02 local authentication, A03 scoped API credentials and A04 GitHub sign-in implemented. C06's audited resolver gaps are closed with bounded Linux descriptor-rooted hashing. The [Parts 01–02 audit](docs/parts-01-02-audit.md) records remaining store-lifetime/image-resolution gates before execution. WSL2 benchmark records are development baselines. Local login, sessions, host-local bootstrap/recovery, scoped expiring API credentials and the GitHub sign-in flow exist as library and host-local command surfaces; the HTTP routes that use them (W08) and job execution are not implemented yet.
...
**Audit integration gates (2026-09-14):** C02/W02 must bound reader admission/pool retention and define writer panic/drain/ambiguous-wait behavior, and enforce the single-controller startup contract. C05/W03 must durably resolve image digest/platform before executable admission. C06 provides bounded confined hashing; W03 must invoke it on a private pre-execution checkout or immutable snapshot. A01 fixes database ownership and future-version rejection. Details and qualification blockers: [Parts 01–02 audit](docs/parts-01-02-audit.md). These gates do not block identity/bootstrap work.
...
**Next task:** `W01` — expiring single-use worker enrollment, worker-generated identity, scoped pool access, authenticated persistent direct-TLS sessions, heartbeat and protocol negotiation. The first vertical slice's identity prerequisites (`A01`–`A04`) are complete; `A05`–`A08` remain open and do not block execution work.
