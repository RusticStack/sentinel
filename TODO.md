...
**Next task:** `C02` — SQLite migrations, WAL/foreign keys, tenant ownership constraints, indexed ready-queue queries, short transactions, the single blocking writer path and the documented commit acknowledgement policy, persisting the C01 state/fence/cancel/timestamp fields with compare-and-set transitions.
