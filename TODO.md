...
**Next task:** `C06` — bounded expression grammar for `${{ … }}`: event/ref filters, concurrency keys, dependency outcome conditions (`if`), policy defaults and `hash_files` evaluation phases and limits; expressions must never become arbitrary execution or network calls.
